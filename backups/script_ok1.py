...
def main():
    ...
    if not df_merged.empty:
        ...
        line_tide, = ax2.plot(df_merged.index, df_merged['tide'], linestyle='-', color='lightgray', label='Tide Level', linewidth=1.0, alpha=0.9, zorder=1)
        lines.append(line_tide)
    # Configure the secondary y-axis (tide)
    ax2.set_ylabel('Tide Level (m)', fontsize=14, color='dimgray') # Color for label
    ax2.tick_params(axis='y', labelcolor='dimgray') # Color for tick labels
    ax2.grid(False) # Turn off grid for the second y-axis to avoid clutter if not desired
