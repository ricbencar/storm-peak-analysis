...
def main():
    ...
    if not df_merged.empty and 'tide' in df_merged.columns and not df_merged['tide'].dropna().empty:
        ...
        line_tide, = ax2.plot(df_merged.index, df_merged['tide'], linestyle='-', color='lightgray', label='Tide Level', linewidth=1.2, alpha=1.0, zorder=1) 
        ...
        # Add a small padding (e.g., 10% of the range, or a fixed small value if range is tiny)
        padding = tide_range * 0.1 if tide_range > 0.1 else 0.05 
